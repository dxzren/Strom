...
//    [Hotfix]
    public class FilterWord
    {
        ...
        {
            dictionaryPath = path;
            MEMORYLEXICON = new WordGroup[(int)char.MaxValue];
            LoadDictionary();
        }
        ...
        private WordGroup[] MEMORYLEXICON = null;
        ...
        private void LoadDictionary()
        {
            if (DictionaryPath != string.Empty)
            {
                ...
                TextAsset ta = Resources.Load(DictionaryPath) as TextAsset;
                if (ta == null)
                {
                    Debug.LogError("加载 屏蔽字库出错" + dictionaryPath);
                    return;
                }
                string msg = ta.text;
        ...
        public string Filter(char replaceChar, string tmpText)
        {
            sourctText = tmpText;
            cursor = 0;
